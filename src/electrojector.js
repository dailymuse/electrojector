...
class Electrojector {
  ...
  set(target, name, value, receiver) {
    if (name[0] === "$")
      throw new TypeError("names begining with $ are reserved");
    if (this.isDone)
      throw new Error("dependency configuration is already complete");
  ...
  $inject(_name, request) {
    ...
    this.installFault(name, deps => {
      ...
      if (typeof fn !== "function")
        throw new Error(`Module '${name}' doesn't export a function`);
      try {
        return fn(deps);
      } catch (e) {
        e.message = `Couldn't initialize '${name}' - ${e.message.replace(
          "Error: ",
          ""
        )}`;
        throw e;
